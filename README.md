## DevOps

- [Jobs Control: `jobs`, `bg`, `&`, `fg`, `disown`, `nohup`, `kill`](README.d/jobs-control.md)
- [Git 必知必会](README.d/git.md)
- [Bash 交互式编辑](README.d/bash-interactive-editing.md)
- [MySQL CLI](README.d/mysql-cli.md)
- [Java: How Classes are Found](README.d/java-how-classes-are-found)

## Config

- [在 MacOS, 启动终端时自动 `cd` 到 Finder 已打开的目录](README.d/auto-working-directory.md)
- [Windows 网页打开本地应用](README.d/windows-webpage-open-application.md)
- [Setup wireguard Server and Clients](README.d/setup-wireguard-server-and-clients.md)
- [办公室旧电脑安装 Ubuntu Server 19.04](README.d/ubuntu-server-19.04-on-old-pc.md)
- [Ubuntu PPTP VPN Server](README.d/ubuntu-pptp-server.md)
- [SSH 远程登录基本配置](README.d/all-about-ssh-remote-logging.md)
- [`brew`, `npm` 等国内源及代理](README.d/all-mirrors-and-sources.md)

## Lingos

- [TypeScript](https://github.com/ngolin/typescript-handbook-refresher/blob/master/README.md)
- [JavaScript](README.d/pl-javascript.md)
- [Rust](https://github.com/ngolin/rust-book-refresher/blob/master/README.md)
